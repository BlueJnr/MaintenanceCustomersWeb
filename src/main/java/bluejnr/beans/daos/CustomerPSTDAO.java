...
public class CustomerPSTDAO implements CustomerService {
    ...
    public CustomerTO createCustomer(CustomerTO customer) {
        int filas = 0;
        try {
            String sql = "insert into clientes (apellidos, nombres, correo, telefono, total_compras) values(?,?,?,?,?)";
            ...
            pst.setDouble(5, customer.getTotalPurchases());
    ...
    public CustomerTO updateCustomer(CustomerTO customer) {
        int filas = 0;
        try {
            String sql = "update clientes set apellidos=?, nombres=?, correo=?, telefono=?, total_compras=? where idcliente=?";
            con = DBConn.getConnection();
            pst = con.prepareStatement(sql);
            pst.setString(1, customer.getSurnames());
            pst.setString(2, customer.getNames());
            ...
            pst.setString(4, customer.getTelephone());
            pst.setDouble(5, customer.getTotalPurchases());
            pst.setInt(6, customer.getIdCustomer());
            filas = pst.executeUpdate();
        } catch (SQLException ex) {
            System.out.println(Util.error1);
        }
    ...
    public List<CustomerTO> getCustomers() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                customer.setTotalPurchases(Double.parseDouble(rs.getString(6)));
